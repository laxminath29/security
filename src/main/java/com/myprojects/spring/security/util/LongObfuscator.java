package com.myprojects.spring.security.util;
...
public class LongObfuscator {
	...
	public final Long obfuscate(final Long number) {
		return modulus(BigInteger.valueOf(number).multiply(OBFUSCATION_FACTOR).remainder(OBFUSCATION_PIVOT),
				OBFUSCATION_PIVOT).longValue();
	}

	public final Long unobfuscate(final Long number) {
		return modulus(BigInteger.valueOf(number).multiply(OBFUSCATION_INVERSE).remainder(OBFUSCATION_PIVOT),
				OBFUSCATION_PIVOT).longValue();
	}
